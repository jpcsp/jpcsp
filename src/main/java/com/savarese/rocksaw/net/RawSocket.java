...
/*
 * Original file copied from rocksaw library.
 * Modified by gid15 for use in Jpcsp: changed all "private" fields and
 * "private" method to "protected", so that this class can be extended
 * by SelectableRawSocket.
 * I preferred to change as little as possible in this
 * file and make changes in an extended class.
 * 
 */
...
//      throw new UnsatisfiedLinkError(__getErrorMessage());
  ...
  protected int __socket;
  protected int __family;
  protected TimeVal __stimeout, __rtimeout;
   ...
   */
  protected native static void __getErrorMessage(StringBuffer buffer);
  ...
  protected static void __throwInterruptedIOException()
    throws InterruptedIOException
  {
    throw new InterruptedIOException(__getErrorMessage());
  }

  protected native static int __socket(int protocolFamily, int protocol);
  ...
  protected native static int __bindDevice(int socket, String device);
  ...
  protected native static int __close(int socket);
  ...
  protected native static int __setIPHeaderInclude(int socket, boolean on);

  /**
   * @return A negative value if an error occurs, else zero for false and
   *         a positive value for true.
   */
  protected native static int __getIPHeaderInclude(int socket);
  ...
  protected native static int __setSendBufferSize(int socket, int size);
  ...
  protected native static int __getSendBufferSize(int __socket);
  ...
  protected native static int __setReceiveBufferSize(int socket, int size);
  ...
  protected native static int __getReceiveBufferSize(int socket);
  ...
  protected native static int __setSendTimeout(int socket, int timeout);
  ...
  protected native static int __getSendTimeout(int socket);
  ...
  protected native static int __setReceiveTimeout(int socket, int timeout);
  ...
  protected native static int __getReceiveTimeout(int socket);
