...
public class GeList extends AbstractNativeCodeSequence {
	static public void updateCommands(int base, int startReg, int endReg, int offsetReg, int stepReg) {
		...
		int skip = (step - 4) >> 2;
		IMemoryReader baseReader = getMemoryReader(getRegisterValue(base), (end - start) << 4, 4);
		for (int i = start; i < end; i++) {
			baseReader.skip(1);
			int addr = baseReader.readNext();
			int count = baseReader.readNext();
			int dest = baseReader.readNext();
			IMemoryReader addrReader = getMemoryReader(addr, count << 2, 4);
			IMemoryWriter destWriter = getMemoryWriter(dest + offset, count * step, 4);
			for (int j = 0; j < count; j++) {
				int src = addrReader.readNext();
				destWriter.writeNext(read32(src));
