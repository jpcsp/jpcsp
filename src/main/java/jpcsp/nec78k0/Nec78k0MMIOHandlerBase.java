 ...
 *
 */
public class Nec78k0MMIOHandlerBase extends MMIOHandlerBase {
	protected Nec78k0Processor processor;

	protected Nec78k0MMIOHandlerBase(int baseAddress) {
		super(baseAddress);
	}
    ...
    private void invalidMemoryAddress(int address, String value, String prefix, int status) {
        log.error(String.format("0x%04X - %s - Invalid memory address: 0x%04X%s", getPc(), prefix, address, value));
