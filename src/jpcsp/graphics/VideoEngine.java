...
import static jpcsp.HLE.modules.sceGe_user.PSP_GE_LIST_END_REACHED;
import static jpcsp.HLE.modules.sceGe_user.PSP_GE_LIST_STALL_REACHED;
import static jpcsp.HLE.modules.sceGe_user.PSP_GE_MATRIX_BONE0;
import static jpcsp.HLE.modules.sceGe_user.PSP_GE_MATRIX_BONE1;
import static jpcsp.HLE.modules.sceGe_user.PSP_GE_MATRIX_BONE2;
import static jpcsp.HLE.modules.sceGe_user.PSP_GE_MATRIX_BONE3;
import static jpcsp.HLE.modules.sceGe_user.PSP_GE_MATRIX_BONE4;
import static jpcsp.HLE.modules.sceGe_user.PSP_GE_MATRIX_BONE5;
import static jpcsp.HLE.modules.sceGe_user.PSP_GE_MATRIX_BONE6;
import static jpcsp.HLE.modules.sceGe_user.PSP_GE_MATRIX_BONE7;
import static jpcsp.HLE.modules.sceGe_user.PSP_GE_MATRIX_PROJECTION;
import static jpcsp.HLE.modules.sceGe_user.PSP_GE_MATRIX_TEXGEN;
...
public class VideoEngine {
    ...
    private class HideEffectsSettingsListener extends AbstractBoolSettingsListener {

        @Override
        protected void settingsValueChanged(boolean value) {
            setHideEffects(value);
        }
    }
    ...
    public void onStallAddrUpdated(PspGeList list, int oldStallAddr) {
        if (State.recordGeFrames) {
        	if (log.isDebugEnabled()) {
        		log.debug(String.format("Capture List after update stall address oldStallAddr=0x%08X, newStallAddr=0x%08X", oldStallAddr, list.getStallAddr()));
        	}
    ...
    public void start() {
        ...
        Settings.getInstance().registerSettingsListener(name, "emu.hideEffects", new HideEffectsSettingsListener());
    ...
    public boolean update() {
        ...
        if (State.dumpGeNextFrame) {
        	...
        	if (!CaptureManager.captureInProgress) {
        		CaptureManager.startCapture(mem, replayFileName, currentList);
        	} else {
        		CaptureManager.startListCapture(mem, currentList);
        	}
        }
    ...
    private boolean tryToFallback() {
        ...
        if (!currentList.isStackEmpty()) {
            ...
            currentList.pushFinishCallback(0);
            listHasEnded = true;
            abort = true;
        }
    ...
    private void executeList() {
        ...
        waitForSyncCount = 0;
        while (!listHasEnded && (!Emulator.pause || State.dumpGeNextFrame || State.recordGeFrames)) {
    ...
    private Buffer unswizzleTextureFromMemory(int texaddr, int bytesPerPixel, int level, int textureBufferWidthInPixels) {
        ...
        if (State.recordGeFrames) {
            log.debug("Capture unswizzleTextureFromMemory");
    ...
    private boolean isFrameBufferTexture() {
    	// Do we have a texture enabled?
    	// Clear mode has also no texture.
        if (!context.textureFlag.isEnabled() || context.clearMode) {
        	return false;
        }

        // Frame buffer textures can only be stored in VRAM
    	if (!isVRAM(context.texture_base_pointer[0])) {
    		return false;
    	}

    	// Not a frame buffer if this is a video texture (e.g. generated by sceMpeg)
    	if (isVideoTexture(context.texture_base_pointer[0])) {
    		return false;
    	}

    	// A frame buffer texture cannot be swizzled
    	if (context.texture_swizzle) {
    		return false;
    	}

    	// A frame buffer texture cannot be compressed
    	if (context.texture_storage >= TPSM_PIXEL_STORAGE_MODE_DXT1 && context.texture_storage <= TPSM_PIXEL_STORAGE_MODE_DXT5) {
    		return false;
    	}

    	// A frame buffer texture cannot be in 4-bit indexed mode
    	if (context.texture_storage == TPSM_PIXEL_STORAGE_MODE_4BIT_INDEXED) {
    		return false;
    	}
    ...
    private void executeCommandPRIM() {
        ...
        if (hideEffects && (isFrameBufferTexture() || context.fbw < Screen.width)) {
        	if (isLogDebugEnabled) {
        		log(String.format("Skipping %s for FB texture 0x%08X, FBW=0x%X", helper.getCommandString(PRIM), context.texture_base_pointer[0], context.fbw));
        	}
        	return;
        }
    ...
    private void executeCommandBJUMP() {
        ...
        if (takeConditionalJump) {
            ...
            if (State.recordGeFrames) {
                log.debug("Capture List after taking BJUMP");
            ...
            if (State.recordGeFrames) {
                log.debug("Capture List after not taking BJUMP");
    ...
    private void executeCommandNOP() {
        ...
        // The check on the nopCount is currently disabled as it is causing issues in "Dissidia 012: Duodecim Final Fantasy".
        if (listCount > 2 && nopCount > Integer.MAX_VALUE) {
    ...
    private void executeCommandJUMP() {
        ...
        if (State.recordGeFrames) {
            log.debug("Capture List after JUMP");
            CaptureManager.captureList(mem, currentList);
        }
    }
    ...
    private void executeCommandCALL() {
        ...
        if (State.recordGeFrames) {
            log.debug("Capture List after CALL");
            CaptureManager.captureList(mem, currentList);
        }
    }
    ...
    private void executeCommandRET() {
        ...
        if (State.recordGeFrames) {
            log.debug("Capture List after RET");
            CaptureManager.captureList(mem, currentList);
        }
    }
    ...
    private void executeCommandEND() {
    	if (State.replayGeFrames) {
    		...
    		if (isLogDebugEnabled) {
    			log.debug(String.format("%s/%s at pc=0x%08X", helper.getCommandString(previousCommand), helper.getCommandString(END), currentList.getPc() - 4));
    		}
    	...
    	if (lleRun) {
	        ...
	        if (previousCommand == SIGNAL || previousCommand == FINISH) {
	            ...
	            currentList.pauseList();
	            if (isLogDebugEnabled) {
	                log(String.format("%s/%s pc=0x%08X", helper.getCommandString(previousCommand), helper.getCommandString(END), currentList.getPc()));
	            }
	            updateGeBuf();

	            if (State.recordGeFrames) {
	            	CaptureManager.captureGEDetails(previousCommand == FINISH);
	            }
    ...
    private void executeCommandSIGNAL() {
        ...
        switch (behavior) {
            ...
            case sceGe_user.PSP_GE_SIGNAL_HANDLER_PAUSE: {
                currentList.clearRestart();
                currentList.pushSignalCallback(behavior, signal);
                break;
            }
            default: {
                if (isLogWarnEnabled) {
    ...
    private void loadTexture(int level, int reTextureLevel) {
            ...
            switch (context.texture_storage) {
                    ...
                    switch (context.tex_clut_mode) {
                        ...
                        case CMODE_FORMAT_16BIT_ABGR4444: {
                            ...
                            if (!context.texture_swizzle) {
                                ...
                                if (State.recordGeFrames) {
                                    log.debug("Capture loadTexture clut 4/16 unswizzled");
                        ...
                        case CMODE_FORMAT_32BIT_ABGR8888: {
                            ...
                            if (!context.texture_swizzle) {
                                ...
                                if (State.recordGeFrames) {
                                    log.debug("Capture loadTexture clut 4/32 unswizzled");
                                    CaptureManager.captureRAM(mem, texaddr, length / 2);
                                }
                            } else {
                                unswizzleTextureFromMemory(texaddr, 0, level, textureBufferWidthInPixels);
                ...
                case TPSM_PIXEL_STORAGE_MODE_16BIT_ABGR4444: {
                    ...
                    if (!context.texture_swizzle) {
                        ...
                        if (State.recordGeFrames) {
                            log.debug("Capture loadTexture 16 unswizzled");
                            CaptureManager.captureRAM(mem, texaddr, length * 2);
                        }
                    } else {
        ...
        if (State.dumpGeNextFrame) {
            ...
            if (vramImage || !CaptureManager.isImageCaptured(texaddr)) {
                CaptureManager.dumpImage(texaddr, level, final_buffer, context.texture_width[level], context.texture_height[level], context.texture_buffer_width[level], buffer_storage, compressedTexture, compressedTextureSize, false, overwriteFile);
            }
        }
    }
    ...
    private void loadTexture() {
        ...
        if (texture == null || !texture.isLoaded() || State.dumpGeNextFrame || State.recordGeFrames) {
            if (isLogDebugEnabled) {
            ...
            if (tex_addr == context.fbp && context.texture_storage == context.psm && context.texture_buffer_width[mipmapIndex] == context.fbw) {
            	if (hideEffects) {
            		if (isLogDebugEnabled) {
            			log(String.format("Skipping copy of GE to Memory for 0x%08X", tex_addr));
            		}
            		textureChanged = false;
            		return;
            	}
        ...
        switch (context.tex_clut_mode) {
            ...
            case CMODE_FORMAT_16BIT_ABGR4444: {
                ...
                if (!context.texture_swizzle) {
                    ...
                    if (State.recordGeFrames) {
                        log.debug("Capture loadTexture clut 8/16 unswizzled");
            ...
            case CMODE_FORMAT_32BIT_ABGR8888: {
                ...
                if (!context.texture_swizzle) {
                    ...
                    if (State.recordGeFrames) {
                        log.debug("Capture loadTexture clut 8/32 unswizzled");
                        CaptureManager.captureRAM(mem, texaddr, length * bytesPerIndex);
                    }
                } else {
                    unswizzleTextureFromMemory(texaddr, bytesPerIndex, level, textureBufferWidthInPixels);
    ...
    private Buffer getTextureBuffer(int texaddr, int bytesPerPixel, int level, int textureBufferWidthInPixels) {
        ...
        if (!context.texture_swizzle) {
            ...
            if (State.recordGeFrames) {
                log.debug("Capture getTextureBuffer unswizzled");
    ...
    private void updateGeBuf() {
        if (geBufChanged) {
        	if (hideEffects && context.fbw < Screen.width) {
        		if (isLogDebugEnabled) {
        			log(String.format("Skipping copy GE to Memory fbw=0x%X", context.fbw));
        		}
        	} else {
    ...
    public void setHideEffects(boolean hideEffects) {
    	if (this.hideEffects != hideEffects) {
    		if (isLogInfoEnabled) {
    			log.info(String.format("setHideEffects %b", hideEffects));
    		}
