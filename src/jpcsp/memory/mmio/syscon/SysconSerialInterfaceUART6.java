...
public class SysconSerialInterfaceUART6 implements IState {
	private static Logger log = Nec78k0Processor.log;
	...
	private void readSecureFlash(int address, int[] array, int offset) {
		array[offset + 0] = sfr.getSysconSecureFlash().read8(address + 2);
		array[offset + 1] = sfr.getSysconSecureFlash().read8(address + 3);
		array[offset + 2] = sfr.getSysconSecureFlash().read8(address + 0);
		array[offset + 3] = sfr.getSysconSecureFlash().read8(address + 1);
	}

	private void createDynamicKey(int[] secret, int[] key, int[] challenge1secret, int[] challenge2secret) {
		final int[] keyF600 = new int[16];
		for (int i = 0; i < 8; i += 4) {
			readSecureFlash(0x0790 + i, keyF600, i);
		}
		...
		for (int i = 0; i < 8; i += 4) {
			readSecureFlash(0x0798 + i, keyF610, i + 8);
		}

		final int[] keyF5E0 = new int[32];
		for (int i = 0; i < 16; i += 4) {
			readSecureFlash(0x07C0 + i, keyF5E0, i);
		}
	...
	private String intsToString(int[] a, int start, int length) {
	...
	private String intsToString(int[] a) {
	...
	private void endReceptionBuffer() {
		...
		if (log.isDebugEnabled()) {
			log.debug(String.format("UART6 prepared reception buffer: %s", intsToString(buffer, 0, receptionBufferSize)));
		}
	...
	private void addReceptionBufferData8(int[] data, int offset, int length) {
		if (data != null) {
			for (int i = 0; i < length; i++) {
	...
	private void executeSysconCmdBattery(int batteryCommand, int sysconCmdBattery, int length, int transmissionLength) {
		switch (sysconCmdBattery) {
			...
			case 0xE0:
				if (length == 11) {
					...
					if (log.isDebugEnabled()) {
						log.debug(String.format("UART6 received syscon command %s, keyId=0x%02X, challenge=%s", getSysconCmdName(sysconCmdBattery), keyId, intsToString(buffer, 4, 8)));
					}
					...
					log.error(String.format("UART6 invalid length=%d for received syscon command %s", length, getSysconCmdName(sysconCmdBattery), intsToString(buffer, 0, transmissionLength)));
				}
				break;
			case 0xE1:
				if (length == 10) {
					if (log.isDebugEnabled()) {
						log.debug(String.format("UART6 received syscon command %s, unused data=%s", getSysconCmdName(sysconCmdBattery), intsToString(buffer, 3, 8)));
					}
					int[] key = getKey(keyId);
					if (key != null) {
				...
				break;
			default:
				log.error(String.format("UART6 setOperationMode starting reception for unknown battery command 0x%02X(%s): %s", batteryCommand, getSysconCmdName(sysconCmdBattery), intsToString(buffer, 0, transmissionLength)));
				break;
		}
	}
				...
				if (command == 0x5A) {
					if (length + 2 == transmissionLength) {
						...
						if (isValidChecksum(checksum, length + 1)) {
							...
							log.error(String.format("UART6 setOperationMode invalid checksum 0x%02X: %s", checksum, intsToString(buffer, 0, transmissionLength)));
						}
					} else {
						log.error(String.format("UART6 setOperationMode starting reception for unknown command length 0x%02X: %s", length, intsToString(buffer, 0, transmissionLength)));
					}
				} else {
					log.error(String.format("UART6 setOperationMode starting reception for unknown command 0x%02X: %s", command, intsToString(buffer, 0, transmissionLength)));
				}
			} else {
				log.error(String.format("UART6 setOperationMode starting reception for unknown command buffer: %s", intsToString(buffer, 0, transmissionLength)));
			}
		}
