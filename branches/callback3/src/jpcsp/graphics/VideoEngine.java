...
public class VideoEngine {
        ...
        switch (command) {
            ...
            case CLEAR:
            	if(clearMode && (normalArgument & 1) == 0) {
            		...
            		if(!useShaders) {
                        // I keep forgetting which way round it goes (fiveofhearts)
                        // - comment out for TOE
                        // - uncomment for cube sample
    ...
    private void loadTexture() {
        ...
        if (!useTextureCache || (tex_addr >= MemoryMap.START_VRAM && tex_addr <= MemoryMap.END_VRAM && texture_storage <= TPSM_PIXEL_STORAGE_MODE_32BIT_ABGR8888)) {
            texture = null;
