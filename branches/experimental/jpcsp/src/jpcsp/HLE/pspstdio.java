...
public class pspstdio {
   ...
   {
       PSPThread.currentPSPThread().processor.gpr[2] =3;//not sure if it returns okay but seems so
   }
   public void sceKernelStdout ()//Function to get the current standard out file no.
   {
	   PSPThread.currentPSPThread().processor.gpr[2] =1;//not sure if it returns okay but seems so
   }
   public void sceKernelStderr ()//Function to get the current standard err file no. 
   {
	   PSPThread.currentPSPThread().processor.gpr[2] =2;//not sure if it returns okay but seems so
   }
