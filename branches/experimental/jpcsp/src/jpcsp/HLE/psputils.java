...
public class psputils {
    ...
    public void Initialise() {
        ...
        initialclocks = System.currentTimeMillis(); // seconds * 1 million
    ...
    public void sceKernelLibcTime(int time_t_addr) {
        ...
        if (time_t_addr != 0)
            Memory.write32(time_t_addr, seconds);
        PSPThread.currentPSPThread().processor.gpr[2] = seconds;
    }

    int currentClocks = 0;
    public void sceKernelLibcClock() {
        currentClocks += 5000;
        PSPThread.currentPSPThread().processor.gpr[2] = currentClocks;
    }
    ...
    public void sceKernelUtilsMt19937Init(int ctx_addr, int seed) {
        ...
        for (int i = 0; i < 628; i += 4) {
            Memory.write32(ctx_addr + i, 0xcdcdcdcd);
        }
    ...
    public void sceKernelUtilsMt19937UInt(int ctx_addr) {
        ...
        if (ctx != null) {
        	PSPThread.currentPSPThread().processor.gpr[2] = ctx.r.nextInt();
        } else {
