...
import jpcsp.Debugger.DisassemblerModule.syscalls;

public class SyscallHandler {

    // Change this to return the number of cycles used?
    public static void syscall(int code) {
    	PSPThread thread = (PSPThread) Thread.currentThread();
        int gpr[] = thread.processor.gpr;
		...
		// sceRtcParseRFC3339(0x2123),
                case 0x2124:
                	pspAudio.get_instance().sceAudioOutput(gpr[4], gpr[5], gpr[6]);
                	ThreadMan.get_instance().ThreadMan_sceKernelDelayThread(0);
                	break;
                case 0x2125:
					ThreadMan.get_instance().ThreadMan_sceKernelDelayThread(0); 
					pspAudio.get_instance().sceAudioOutputBlocking(gpr[4], gpr[5], gpr[6]);
					ThreadMan.get_instance().ThreadMan_sceKernelDelayThread(0);
					break;
				case 0x2126:
					pspAudio.get_instance().sceAudioOutputPanned(gpr[4], gpr[5], gpr[6], gpr[7]);
					ThreadMan.get_instance().ThreadMan_sceKernelDelayThread(0);
					break;
                case 0x2127:
