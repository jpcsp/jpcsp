...
public static String ElfInfo, ProgInfo, PbpInfo, SectInfo;
    ...
    private void initCpuBy(Elf32 elf) {
    	...
    	int entry = (int) romManager.getBaseoffset() + (int) elf.getHeader().getE_entry(); //set the pc register.    	
    	int gp_addr = (int) romManager.getBaseoffset() + (int) romManager.getPSPModuleInfo().getM_gp();

        jpcsp.HLE.pspdisplay.get_instance().Initialise();
        jpcsp.HLE.ThreadMan.get_instance().Initialise(entry, gp_addr, romManager.getPSPModuleInfo().getM_attr());
